...
public function getLastid($table){
			$sql="SELECT * FROM ".$table." ORDER BY stock_id ASC LIMIT 0,1";
			return $sql;
		}

		public function genererAffichageAllstock(){
			$sql="SELECT MAX(stock_id),st.initial_balance , MAX(date) as date, SUM(stock_in) AS stock_in,SUM(stock_out) AS stock_out, total_stock.total as balance, sousca.description as sous_category_description,cat.description as category_description FROM stock as st INNER JOIN sous_category as sousca ON ( st.sous_category_id=sousca.sous_category_id) INNER JOIN category as cat ON ( st.category_id=cat.category_id) INNER JOIN total_stock ON(total_stock.category_id=cat.category_id AND total_stock.sous_category_id=sousca.sous_category_id) WHERE stock_id=(SELECT max(stock_id) FROM stock) ";
			return $sql;
		}

		public function genererAffichageAllstock_date(){
			$sql="SELECT st.initial_balance ,MAX(date) as date, SUM(stock_in) AS stock_in,SUM(stock_out) AS stock_out, total_stock.total as balance, sousca.description as sous_category_description,cat.description as category_description FROM stock as st INNER JOIN sous_category as sousca ON ( st.sous_category_id=sousca.sous_category_id) INNER JOIN category as cat ON ( st.category_id=cat.category_id) INNER JOIN total_stock ON(total_stock.category_id=cat.category_id AND total_stock.sous_category_id=sousca.sous_category_id) AND st.date BETWEEN '10-10-2017' AND '26-10-2017' ";
			return $sql;
		}
