<?php
	...
	class Daostock extends Dao {
		...
		public function genererAffichageAllstock_cat_sub($cat, $sub_cat){
			$sql="SELECT initial_balance AS initial_balance, st.date, stock_in AS stock_in, stock_out AS stock_out, total_stock.total balance, sousca.description as sous_category_description,cat.description as category_description FROM stock as st INNER JOIN sous_category as sousca ON ( st.sous_category_id=sousca.sous_category_id) INNER JOIN category as cat ON ( st.category_id=cat.category_id) INNER JOIN total_stock ON(total_stock.category_id=cat.category_id AND total_stock.sous_category_id=sousca.sous_category_id) where cat.category_id=".$cat." AND sousca.sous_category_id=".$sub_cat."  ORDER BY stock_id DESC LIMIT 20";
			return $sql;
		}

		public function genererAffichageAllstock_per_date($cat, $sub_cat, $date_debut, $date_fin){
			$sql="SELECT SUM(initial_balance) AS initial_balance, st.date, SUM(stock_in) AS stock_in,SUM(stock_out) AS stock_out, total_stock.total balance, sousca.description as sous_category_description,cat.description as category_description FROM stock as st INNER JOIN sous_category as sousca ON ( st.sous_category_id=sousca.sous_category_id) INNER JOIN category as cat ON ( st.category_id=cat.category_id) INNER JOIN total_stock ON(total_stock.category_id=cat.category_id AND total_stock.sous_category_id=sousca.sous_category_id) where cat.category_id=".$cat." AND sousca.sous_category_id=".$sub_cat." AND DATE_FORMAT(STR_TO_DATE(st.date, '%d-%m-%Y'), '%Y-%m-%d') BETWEEN '".$date_debut."' and '".$date_fin."' GROUP BY sousca.sous_category_id ORDER BY stock_id DESC LIMIT 20";
			return $sql;
		}
